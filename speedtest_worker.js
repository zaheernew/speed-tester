...
this.addEventListener("message", function(e) {
	...
	if (params[0] === "status") {
		...
		postMessage(
			JSON.stringify({
				testState: testState,
				dlStatus: 0.98 * dlStatus,
				ulStatus: 0.98 * ulStatus,
	...
	const testStream = function(i, delay) {
		setTimeout(
			function() {
				...
				//Newly Added by seyon 2025-05-19
				xhr[i].setRequestHeader("Cache-Control", "no-cache, no-store, must-revalidate");
				xhr[i].setRequestHeader("Pragma", "no-cache");
				//Newly End
				xhr[i].send();
			}.bind(this),
			1 + delay
